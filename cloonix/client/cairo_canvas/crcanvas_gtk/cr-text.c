...
/****************************************************************************/
static void cr_text_dispose(GObject *object)
{
...
/****************************************************************************/
static void cr_text_finalize(GObject *object)
{
  CrText *text;
  text = CR_TEXT(object);
  if (text->layout)
...
/****************************************************************************/
static void cr_text_set_property(GObject *object, guint property_id,
...
case PROP_FONT:
    if (text->font_desc)
      pango_font_description_free(text->font_desc);
    text->font_desc = pango_font_description_from_string(g_value_get_string (value));
    cr_item_request_update(CR_ITEM(text));
    break;
...
/****************************************************************************/
static void cr_text_get_property(GObject *object, guint property_id,
  ...
  switch (property_id) 
    ...
    case PROP_WIDTH:
      g_value_set_double (value, text->width);
            break;
    case PROP_SCALEABLE:
      g_value_set_boolean(value, text->flags & CR_TEXT_SCALEABLE);
            break;
    case PROP_FONT:
      if (text->font_desc)
        {
    ...
    case PROP_FILL_COLOR_RGBA:
      ...
      break;
    case PROP_FILL_COLOR:
      sprintf(color_string, "#%.6x", text->fill_color_rgba >> 8);
    ...
    case PROP_TEXT:
      ...
      break;
    case PROP_USE_MARKUP:
      g_value_set_boolean(value, text->flags & CR_TEXT_USE_MARKUP);
            break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, property_id, pspec);
    }
...
/****************************************************************************/
static void create_layout(CrText *text, cairo_t *c)
{
  if (!text->layout)
  ...
  if (text->width > 0)
    pango_layout_set_width (text->layout, text->width * PANGO_SCALE);
...
/****************************************************************************/
static void get_device_extents(CrText *text, cairo_t *c, CrDeviceBounds *device)
{
  int width, height;
  double w, h;
  cairo_save(c);
...
/****************************************************************************/
static void get_item_extents(CrText *text, cairo_t *c, 
                             double *x, double *y,
                             double *w, double *h)
{
  int width, height;
...
static void report_new_bounds(CrItem *item, cairo_t *ct, gboolean all)
{
  ...
  if (text->layout && item->bounds && (text->flags & CR_TEXT_SCALEABLE))
    {
    get_item_extents(text, ct, &item->bounds->x1, &item->bounds->y2, &w, &h);
...
/****************************************************************************/
static void paint(CrItem *item, cairo_t *c)
{
  CrText *text;
  double r, g, b, a;
  double x, y;
...
/****************************************************************************/
static gboolean calculate_bounds(CrItem *item, cairo_t *c, CrBounds *bounds,
...
/****************************************************************************/
static CrItem * test(CrItem *item, cairo_t *c, double x, double y)
{
  return item;
}
/*--------------------------------------------------------------------------*/
...
/****************************************************************************/
static void cr_text_init(CrText *text)
{
  text->flags |= CR_TEXT_SCALEABLE;
}
/*--------------------------------------------------------------------------*/
...
/****************************************************************************/
static void cr_text_class_init(CrTextClass *klass)
{
  ...
  g_object_class_install_property(object_class, PROP_WIDTH,
		                  ...
		                  G_PARAM_READWRITE));
  g_object_class_install_property(object_class, PROP_SCALEABLE,
                                  g_param_spec_boolean ("scaleable", NULL,
                                  "When True the size of the text "
                                  "is in item units.  When False, the "
                                  "size is in device/pixel units. "
                                  "This effects x/y_offset as well. "
                                  "See also @CrInverse for another way "
                                  "to achieve the same effect.", TRUE,
		                  G_PARAM_READWRITE));
  g_object_class_install_property(object_class, PROP_X_OFFSET,
                                  g_param_spec_double ("x_offset", NULL,
                                  "A device offset from the item's "
                                  "anchor position.  Only used when "
                                  "scaleable=FALSE.",
		                  -G_MAXDOUBLE, G_MAXDOUBLE, 0.0,
		                  G_PARAM_READWRITE));
  g_object_class_install_property(object_class, PROP_Y_OFFSET,
                                  g_param_spec_double ("y_offset", NULL,
                                  "A device offset from the item's "
                                  "anchor position.  Only used when "
                                  "scaleable=FALSE.", -G_MAXDOUBLE,
  ...
  g_object_class_install_property(object_class, PROP_USE_MARKUP,
                                  g_param_spec_boolean ("use_markup", NULL,
                                  "If html markup language should be used.",
...
static const GTypeInfo info =
  ...
  if (!type)
    type = g_type_register_static(CR_TYPE_ITEM, "CrText", &info, 0);
  return type;
}
/*--------------------------------------------------------------------------*/
                     ...
                     const char *text, const gchar *first_arg_name, ...)
{
  ...
  item = cr_item_construct(parent, CR_TYPE_TEXT, first_arg_name, args);
  va_end (args);
  if (item)
