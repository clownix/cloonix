...
static void node_item_info(GtkWidget *mn, t_item_ident *pm)
{
  ...
  static char title[MAX_PATH_LEN];
  static char text[MAX_TEXT];
  int is_privileged, is_persistent, is_backed, is_natplug;
  ...
  if (bitem)
    ...
    is_privileged = vm_config_flags & VM_CONFIG_FLAG_PRIVILEGED;
...
{
  GtkWidget *item_rsh, *item_info, *item_delete;
//  GtkWidget *item_xephyr;
  GtkWidget *menu = gtk_menu_new();
  t_item_ident *pm = get_and_init_pm(bitem);
  bitem->pbi.menu_on = 1;
  if (bitem->bank_type != bank_type_cnt)
