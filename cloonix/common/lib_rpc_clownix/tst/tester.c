...
static int count_novnc_on_off = 0;
...
/*****************************************************************************/
void recv_novnc_on_off(int llid, int itid, int ion)
{
  static int tid, on;
  if (i_am_client)
    {
    if (count_novnc_on_off)
      {
      if (itid != tid)
        KOUT(" ");
      if (ion != on)
    ...
    tid = rand();
    on = rand();
    send_novnc_on_off(llid, tid, on);
    }
  else
    send_novnc_on_off(llid, itid, ion);
  count_novnc_on_off++;
}
/*---------------------------------------------------------------------------*/
