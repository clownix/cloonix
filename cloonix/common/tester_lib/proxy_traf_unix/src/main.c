...
{ 
  ...
  if ((g_is_server) && (g_llid_traf))
    {
    g_str_tx += len;
    proxy_traf_unix_tx(g_llid_traf, len, buf);
    }
} 
/*--------------------------------------------------------------------------*/
...
static void heartbeat (int delta)
{
  ...
  if ((g_is_server == 0) && (g_llid_traf))
    ...
    for (i=0; i<max; i++)
      {
      proxy_traf_unix_tx(g_llid_traf, strlen(g_str[i]) + 1, g_str[i]);
...
{
  char *sock_path = "/tmp/proxy_traf_unix.sock";
  if (argc != 2)
    {
  ...
  if (g_is_server)
    {
    proxy_traf_unix_server(sock_path, connect_from_traf_client);
    printf("server %s\n", sock_path);
    }
  else
    {
    g_llid_traf = proxy_traf_unix_client(sock_path, traf_err_cb, traf_rx_cb);
    if (g_llid_traf == 0)
      {
      printf("\nBad connection %s\n\n", sock_path);
      exit(-1);
