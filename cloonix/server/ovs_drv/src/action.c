...
{
  if (ovs_cmd_add_lan_rstp(bin, db, lan))
    {
    snprintf(respb, MAX_PATH_LEN-1, "KO ovs_add_lan_rstp lan=%s", lan);
    KERR("%s", respb);
    }
  else
    {
    snprintf(respb, MAX_PATH_LEN-1, "OK ovs_add_lan_rstp lan=%s", lan);
    }
}
/*---------------------------------------------------------------------------*/
