  ...
  if (!strcmp(brandtype, "docker"))
    ...
    if (g_docker_ok)
      snprintf(cmd, MAX_PATH_LEN-1,
             "%s inspect -f \'{{.State.Pid}}\' %s", DOCKER_BIN, id);
    }
  else if (!strcmp(brandtype, "podman"))
    {
    if (g_podman_ok)
      snprintf(cmd, MAX_PATH_LEN-1,
             "%s inspect -f \'{{.State.Pid}}\' %s", PODMAN_BIN, id);
    }
  else
...
/****************************************************************************/
int dockerimages_container_name_already_used(char *brandtype, char *name)
{
...
/****************************************************************************/
void dockerimages_rebuild(void)
{
  g_image_nb = 0;
  g_container_nb = 0;
...
/****************************************************************************/
void dockerimages_update(int llid)
{
  int i;
  char resp[MAX_PATH_LEN];
...
/****************************************************************************/
int dockerimages_get_pid(char *brandtype, char *image_name, char *id)
{
  int i, pid = 0;
  if ((strcmp(brandtype, "docker")) &&  
...
/****************************************************************************/
int dockerimages_exits(char *brandtype, char *name, char *image_id)
{
  int i, result = 0;
  for (i=0; i<g_image_nb; i++)
    {
