...
static void timeout_end_launch(void *data)
{
  ...
  if (tel->end_launch_state == end_launch_state_start)
    {
    dockerimages_rebuild();
    pid = dockerimages_get_pid(tel->brandtype, tel->image, tel->id);
    if (pid == 0)
      {
    ...
    if (pid == 0)
      ...
      "%s exec %s /mnt/cloonix_config_fs/docker_init_starter.sh", bin, tel->id);
...
static void suppress_previous_name(char *bin, char *name)
{
  ...
  snprintf(cmd, MAX_PATH_LEN-1,"%s rm %s", bin, name);
  if (execute_cmd(cmd, SYNCHRO))
    KERR("ERROR %s", cmd);
}
/*--------------------------------------------------------------------------*/
...
/****************************************************************************/
void dockir_recv_poldiag_msg(int llid, int tid, char *line)
{
  char *rx="cloonsuid_docker_req_imgs";
  if (!strncmp(line, rx, strlen(rx)))
    dockerimages_update(llid);
  else
    KERR("%s", line);
}
/*--------------------------------------------------------------------------*/
...
/****************************************************************************/
void dockir_recv_sigdiag_msg(int llid, int tid, char *line)
{
  char id[MAX_NAME_LEN];
  ...
  "image=%s cnt_dir=%s agent=%s launch=",
    ...
    else if (!dockerimages_exits(type, image, id_image))
      {
      KERR("ERROR %s", line);
      snprintf(resp, MAX_PATH_LEN-1,
      "cloonsuid_docker_resp_launch_ko brandtype=%s name=%s vm_id=%d image=%s",
    ...
    else if (dockerimages_container_name_already_used(type, name))
      {
      KERR("ERROR dockerimages_container_name_already_used %s", line);
      snprintf(resp, MAX_PATH_LEN-1,
      "cloonsuid_docker_resp_launch_ko brandtype=%s name=%s vm_id=%d image=%s",
      type, name, vm_id, image);
      }
