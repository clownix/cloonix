...
int dpdk_fmt_tx_add_lan(int tid, char *lan)
{
  ...
  snprintf(cmd, MAX_PATH_LEN-1, "cloonixovs_add_lan lan_name=%s", lan);
...
int dpdk_fmt_tx_del_lan(int tid, char *lan)
{
  ...
  char cmd[MAX_PATH_LEN];
  memset(cmd, 0, MAX_PATH_LEN);
  snprintf(cmd, MAX_PATH_LEN-1, "cloonixovs_del_lan lan_name=%s", lan);
  result = dpdk_ovs_try_send_diag_msg(tid, cmd);
  return result;
}
/*--------------------------------------------------------------------------*/
...
int dpdk_fmt_tx_del_eth(int tid, char *name, int num, int spy)
{
  ...
  char cmd[MAX_PATH_LEN];
  memset(cmd, 0, MAX_PATH_LEN);
  sprintf(cmd, "cloonixovs_del_eth name=%s num=%d spy=%d", name, num, spy);
  result = dpdk_ovs_try_send_diag_msg(tid, cmd);
  return result;
}
/*--------------------------------------------------------------------------*/
...
void dpdk_fmt_rx_rpct_recv_diag_msg(int llid, int tid, char *line)
{
  ...
  if (sscanf(line,
      "KO cloonixovs_add_eth name=%s num=%d spy=%d", name, &num, &spy) == 3)
    {
    KERR("%s", line);
    ...
    dpdk_msg_ack_eth(tid, name, num, 1, 0, "cloonixovs");
      ...
      "KO cloonixovs_del_eth name=%s num=%d spy=%d", name, &num, &spy) == 3)
    {
    KERR("%s", line);
    dpdk_msg_ack_eth(tid, name, num, 0, 1, "cloonixovs");
    }
  else if (sscanf(line,
      "OK cloonixovs_del_eth name=%s num=%d spy=%d", name, &num, &spy) == 3)
    {
    dpdk_msg_ack_eth(tid, name, num, 0, 0, "cloonixovs");
    }
  else if (sscanf(line,
      "KO cloonixovs_add_lan lan_name=%s", lan_name) == 1)
    {
    KERR("%s", line);
    dpdk_msg_ack_lan(tid, lan_name, 1, 1, "cloonixovs");
    }
  else if (sscanf(line,
      "OK cloonixovs_add_lan lan_name=%s", lan_name) == 1)
    {
    dpdk_msg_ack_lan(tid, lan_name, 1, 0, "cloonixovs");
    }
  else if (sscanf(line,
      "KO cloonixovs_del_lan lan_name=%s", lan_name) == 1)
    {
    KERR("%s", line);
    dpdk_msg_ack_lan(tid, lan_name, 0, 1, "cloonixovs");
    }
  else if (sscanf(line,
      "OK cloonixovs_del_lan lan_name=%s", lan_name) == 1)
    {
