...
enum {
     ...
     ovsreq_del_lan_d2d,
     ovsreq_add_lan_ethd,
     ovsreq_add_lan_ethv_cnt,
     ovsreq_add_lan_ethv,
     ovsreq_add_lan_eths,
     ovsreq_del_lan_ethd,
     ovsreq_del_lan_ethv_cnt,
     ovsreq_del_lan_ethv,
     ovsreq_del_lan_eths,
     ovsreq_add_lan_tap,
     ovsreq_del_lan_tap,
...
static void delay_add_lan_endp(void *data)
{
  ...
  if (lan && (lan->refcount > 0))
    ...
    else if (req->type == ovsreq_add_lan_ethv)
      {
      if (strlen(req->vhost) == 0)
        KERR("ERROR %s %s %d", req->lan, req->name, req->num);
      else 
        result = fmt_tx_add_lan_ethv(req->tid, req->lan,
                                     req->name, req->num, req->vhost);
      if (result)
        {
        KERR("ERROR %s %s %d", req->lan, req->name, req->num);
        dpdk_kvm_resp_add_lan(1, req->lan, req->name, req->num);
    ...
    else if (req->type == ovsreq_add_lan_ethv_cnt)
      {
      if (strlen(req->vhost) == 0)
        KERR("ERROR %s %s %d", req->lan, req->name, req->num);
      else
        result = fmt_tx_add_lan_ethv(req->tid, req->lan, req->name,
                                     req->num, req->vhost);
      if (result)
        {
        KERR("ERROR %s %s %d", req->lan, req->name, req->num);
        container_resp_add_lan(1, req->lan, req->name, req->num);
        ovsreq_free(req->tid);
        }
      }
    ...
    if (req->time_count < 5)
      ...
      if (req->type == ovsreq_add_lan_ethv_cnt)
        container_resp_add_lan(1, req->lan, req->name, req->num);
      else if ((req->type == ovsreq_add_lan_ethd) ||
               (req->type == ovsreq_add_lan_ethv) ||
...
{ 
  ...
  if (cur == NULL)
    ...
    if (result)
      ...
      {
      lan_alloc(lan);
      ovsreq_alloc(tid, ovsreq_add_lan_tap, lan, name, 0, NULL);
      }
    }
    ...
    {
    result = 0;
    ovsreq=ovsreq_alloc(tid, ovsreq_add_lan_tap, lan, name, 0, NULL);
...
int dpdk_msg_send_add_lan_phy(char *lan, char *name)
{
  ...
  if (cur == NULL)
    ...
    if (result)
      ...
      {
      lan_alloc(lan);
      ovsreq_alloc(tid, ovsreq_add_lan_phy, lan, name, 0, NULL);
      }
    }
  else if (cur->refcount > 0)
    {
    ...
    {
    result = 0;
    ovsreq=ovsreq_alloc(tid, ovsreq_add_lan_phy, lan, name, 0, NULL);
    clownix_timeout_add(5, delay_add_lan_endp,(void *) ovsreq, NULL, NULL);
    }
  return result;
}
...
int dpdk_msg_send_add_lan_ethd(char *lan, char *name, int num)
{
  ...
  if (cur == NULL)
    ...
    if (result)
      ...
      {
      lan_alloc(lan);
      ovsreq_alloc(tid, ovsreq_add_lan_ethd, lan, name, num, NULL);
      }
    }
  else if (cur->refcount > 0) 
    {
    ...
    {
    result = 0;
    ovsreq = ovsreq_alloc(tid, ovsreq_add_lan_ethd, lan, name, num, NULL);
    clownix_timeout_add(5, delay_add_lan_endp,(void *) ovsreq, NULL, NULL);
    }
  return result;
}
...
/****************************************************************************/
int dpdk_msg_send_add_lan_ethv(char *lan, char *name, int num, char *vhost)
{
  int result = -1, tid = utils_get_next_tid();
  t_ovslan *cur = lan_find(lan);
  t_ovsreq *ovsreq;
  if (cur == NULL)
    ...
    if (result)
      ...
      {
      lan_alloc(lan);
      ovsreq_alloc(tid, ovsreq_add_lan_ethv, lan, name, num, vhost);
      }
    }
  else if (cur->refcount > 0)
    {
    if (strlen(vhost) == 0)
      ...
      result = fmt_tx_add_lan_ethv(tid, lan, name, num, vhost);
    if (result)
      KERR("ERROR %s %s %d", lan, name, num);
    else
      ovsreq_alloc(tid, ovsreq_add_lan_ethv, lan, name, num, vhost);
    ...
    {
    result = 0;
    ovsreq = ovsreq_alloc(tid, ovsreq_add_lan_ethv, lan, name, num, vhost);
    clownix_timeout_add(5, delay_add_lan_endp,(void *) ovsreq, NULL, NULL);
    }
  return result;
}
...
/****************************************************************************/
int dpdk_msg_send_add_lan_ethv_cnt(char *lan, char *name, int num, char *vhost)
{
  int result = -1, tid = utils_get_next_tid();
  t_ovslan *cur = lan_find(lan);
  t_ovsreq *ovsreq;
  if (cur == NULL)
    ...
    if (result)
      ...
      {
      lan_alloc(lan);
      ovsreq_alloc(tid, ovsreq_add_lan_ethv_cnt, lan, name, num, vhost);
      }
    }
  else if (cur->refcount > 0)
    {
    ...
    {
    result = 0;
    ovsreq = ovsreq_alloc(tid, ovsreq_add_lan_ethv_cnt, lan,name,num,vhost);
    clownix_timeout_add(5, delay_add_lan_endp,(void *) ovsreq, NULL, NULL);
    }
  return result;
}
...
int dpdk_msg_send_add_lan_eths(char *lan, char *name, int num)
{
  ...
  if (cur == NULL)
    ...
    if (result)
      ...
      {
      lan_alloc(lan);
      ovsreq_alloc(tid, ovsreq_add_lan_eths, lan, name, num, NULL);
      }
    }
  else if (cur->refcount > 0)
    {
    ...
    {
    result = 0;
    ovsreq = ovsreq_alloc(tid, ovsreq_add_lan_eths, lan, name, num, NULL);
    clownix_timeout_add(5, delay_add_lan_endp,(void *) ovsreq, NULL, NULL);
    }
  return result;
}
...
int dpdk_msg_send_add_lan_nat(char *lan, char *name)
{
  ...
  if (cur == NULL)
    ...
    if (result)
      ...
      {
      lan_alloc(lan);
      ovsreq_alloc(tid, ovsreq_add_lan_nat, lan, name,0, NULL);
      }
    }
  else if (cur->refcount > 0)
    {
    ...
    {
    result = 0;
    ovsreq = ovsreq_alloc(tid,ovsreq_add_lan_nat,lan,name,0, NULL);
    clownix_timeout_add(5, delay_add_lan_endp,(void *) ovsreq, NULL, NULL);
    }
  return result;
}
...
int dpdk_msg_send_add_lan_a2b(char *lan, char *name, int num)
{
  ...
  if (cur == NULL)
    ...
    if (result)
      ...
      {
      lan_alloc(lan);
      ovsreq_alloc(tid, ovsreq_add_lan_a2b, lan, name, num, NULL);
      }
    }
  else if (cur->refcount > 0)
    {
    ...
    {
    result = 0;
    ovsreq = ovsreq_alloc(tid, ovsreq_add_lan_a2b, lan, name, num, NULL);
    clownix_timeout_add(5, delay_add_lan_endp,(void *) ovsreq, NULL, NULL);
    }
  return result;
}
...
{   
  ...
  if (cur == NULL)
    ...
    if (result)
      ...
      {
      lan_alloc(lan);
      ovsreq_alloc(tid, ovsreq_add_lan_d2d, lan, name, 0, NULL);
      }
    }
  else if (cur->refcount > 0)
    {
    ...
    {
    result = 0;
    ovsreq = ovsreq_alloc(tid,ovsreq_add_lan_d2d,lan,name,0, NULL);
    clownix_timeout_add(5, delay_add_lan_endp,(void *) ovsreq, NULL, NULL);
    }
  return result;
}
...
int dpdk_msg_send_del_lan_tap(char *lan, char *name)
{
  ...
  if (result)
    ...
    ovsreq_alloc(tid, ovsreq_del_lan_tap, lan, name, 0, NULL);
  return result;
}
/*--------------------------------------------------------------------------*/
...
int dpdk_msg_send_del_lan_phy(char *lan, char *name)
{
  ...
  if (result)
    ...
    ovsreq_alloc(tid, ovsreq_del_lan_phy, lan, name, 0, NULL);
  return result;
}
/*--------------------------------------------------------------------------*/
...
int dpdk_msg_send_del_lan_ethd(char *lan, char *name, int num)
{
  ...
  if (result)
    ...
    ovsreq_alloc(tid, ovsreq_del_lan_ethd, lan, name, num, NULL);
  return result;
}
/*--------------------------------------------------------------------------*/

/****************************************************************************/
int dpdk_msg_send_del_lan_ethv(char *lan, char *name, int num, char *vhost)
{
  int result = -1, tid = utils_get_next_tid();
  if (!lan_find(lan))
    KERR("ERROR %s %s %d", lan, name, num);
  else if (strlen(vhost) == 0)
    KERR("ERROR %s %s %d", lan, name, num);
  else
    result = fmt_tx_del_lan_ethv(tid, lan, name, num, vhost);
  if (result)
    KERR("ERROR %s %s %d", lan, name, num);
  else
    ovsreq_alloc(tid, ovsreq_del_lan_ethv, lan, name, num, vhost);
  return result;
}
/*--------------------------------------------------------------------------*/

/****************************************************************************/
int dpdk_msg_send_del_lan_ethv_cnt(char *lan, char *name, int num, char *vhost)
{
  int result = -1, tid = utils_get_next_tid();
  if (!lan_find(lan))
    KERR("ERROR %s %s %d", lan, name, num);
  ...
  if (result)
    ...
    ovsreq_alloc(tid, ovsreq_del_lan_ethv_cnt, lan, name, num, vhost);
  return result;
}
/*--------------------------------------------------------------------------*/
...
int dpdk_msg_send_del_lan_eths(char *lan, char *name, int num)
{
  ...
  if (result)
    ...
    ovsreq_alloc(tid, ovsreq_del_lan_eths, lan, name, num, NULL);
  return result;
}
/*--------------------------------------------------------------------------*/
...
int dpdk_msg_send_del_lan_nat(char *lan, char *name)
{
  ...
  if (result)
    ...
    ovsreq_alloc(tid, ovsreq_del_lan_nat, lan, name, 0, NULL);
  return result;
}
/*--------------------------------------------------------------------------*/
...
int dpdk_msg_send_del_lan_a2b(char *lan, char *name, int num)
{
  ...
  if (result)
    ...
    ovsreq_alloc(tid, ovsreq_del_lan_a2b, lan, name, num, NULL);
  return result;
}
/*--------------------------------------------------------------------------*/
...
int dpdk_msg_send_del_lan_d2d(char *lan, char *name)
{
  ...
  if (result)
    ...
    ovsreq_alloc(tid, ovsreq_del_lan_d2d, lan, name, 0, NULL);
  return result;
}
/*--------------------------------------------------------------------------*/
...
static void transmit_add_ack(int tid, t_ovsreq *cur, int is_ko)
{
  ...
  if (cur->type == ovsreq_add_lan_ethv_cnt)
    {
    container_resp_add_lan(is_ko, cur->lan, cur->name, cur->num);
    }
  else if ((cur->type == ovsreq_add_lan_ethd) ||
           (cur->type == ovsreq_add_lan_ethv) ||
...
static void transmit_del_ack(int tid, t_ovsreq *cur, int is_ko)
{
  ...
  if (lan_refcount_dec(cur->lan, __LINE__) == 1)
    ...
    if ((cur->type == ovsreq_add_lan_ethv) ||
        (cur->type == ovsreq_del_lan_ethv) ||
        (cur->type == ovsreq_add_lan_ethv_cnt) ||
        (cur->type == ovsreq_del_lan_ethv_cnt))
      ovsreq_alloc(ntid, cur->type, cur->lan, cur->name, cur->num, cur->vhost);
    else
      ovsreq_alloc(ntid, cur->type, cur->lan, cur->name, cur->num, NULL);
  ...
  if (cur->type == ovsreq_del_lan_ethv_cnt)
    {
    container_resp_del_lan(is_ko, cur->lan, cur->name, cur->num);
    }
  else if ((cur->type == ovsreq_del_lan_ethd) ||
           (cur->type == ovsreq_del_lan_ethv) ||
    ...
    if (is_add)
      ...
      if (is_ko)
        ...
        if ((cur->type == ovsreq_add_lan_ethv)     ||
            ...
            (cur->type == ovsreq_add_lan_ethv_cnt) ||
            (cur->type == ovsreq_del_lan_ethv_cnt))
          ovsreq = ovsreq_alloc(ntid, cur->type, cur->lan,
                                cur->name, cur->num, cur->vhost);
        else
          ovsreq = ovsreq_alloc(ntid, cur->type, cur->lan,
                                cur->name, cur->num, NULL);
        ovsreq_free(tid);
    ...
    if (cur->time_count > 50)
      ...
      switch(cur->type)
       ...
       case ovsreq_add_lan_d2d:
        ...
        case ovsreq_add_lan_ethv_cnt:
          KERR("ERROR TIMEOUT %d %s %s %d", cur->tid, cur->lan,
                                            cur->name, cur->num);
          container_resp_add_lan(1, cur->lan, cur->name, cur->num);
          ovsreq_free(cur->tid);
        break;

        case ovsreq_del_lan_ethv_cnt:
          KERR("ERROR TIMEOUT %d %s %s %d", cur->tid, cur->lan,
                                      cur->name, cur->num);
          container_resp_del_lan(1, cur->lan, cur->name, cur->num);
          ovsreq_free(cur->tid);
        break;
