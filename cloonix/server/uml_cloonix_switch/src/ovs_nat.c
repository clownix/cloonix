...
static t_ovs_nat *g_head_nat;
static int g_nb_nat;

int get_glob_req_self_destruction(void);

/****************************************************************************/
static t_ovs_nat *find_nat(char *name)
{
...
static void alloc_nat(int cli_llid, int cli_tid, char *name)
{
  ...
  t_ovs_nat *cur = (t_ovs_nat *) malloc(sizeof(t_ovs_nat));
  memset(cur, 0, sizeof(t_ovs_nat));
  ...
  mc[4] = cur->nat_id % 100;
  mc[5] = 0xFF & rand();
  snprintf(cur->socket, MAX_NAME_LEN-1, "%s/%s/c%s", root, NAT_DIR, name);
...
static int try_connect(char *socket, char *name)
{
  ...
  if (llid)
    {
    if (hop_event_alloc(llid, type_hop_nat, name, 0))
      KERR(" ");
    llid_trace_alloc(llid, name, 0, 0, type_llid_trace_endp_ovsdb);
    rpct_send_pid_req(llid, type_hop_nat, name, 0);
    }
  return llid;
}
/*--------------------------------------------------------------------------*/
...
static void timer_heartbeat(void *data)
{
  ...
  while(cur)
    ...
    if (cur->llid == 0)
      ...
      if (cur->count == 5)
        {
        rpct_send_pid_req(cur->llid, type_hop_nat, cur->name, 0);
        cur->count = 0;
        }
...
/****************************************************************************/
void ovs_nat_resp_add_lan(int is_ko, char *name, int num,
                         char *vhost, char *lan)
{
  t_ovs_nat *cur = find_nat(name);
  if (!cur)
...
/****************************************************************************/
void ovs_nat_resp_del_lan(int is_ko, char *name, int num,
                          char *vhost, char *lan)
{
  t_ovs_nat *cur = find_nat(name);
  if (!cur)
...
/****************************************************************************/
void ovs_nat_add(int cli_llid, int cli_tid, char *name)
{
  t_ovs_nat *cur = find_nat(name);
  if (cur)
    {
...
/****************************************************************************/
void ovs_nat_del(int cli_llid, int cli_tid, char *name)
{
  t_ovs_nat *cur = find_nat(name);
  int val;
  if (!cur)
...
/****************************************************************************/
void ovs_nat_del_lan(int cli_llid, int cli_tid, char *name, char *lan)
{
  t_ovs_nat *cur = find_nat(name);
  int val = 0;
  if (!cur)
...
void ovs_nat_init(void)
{
  ...
  char *bin_nat = utils_get_ovs_nat_bin_dir();
