...
/*****************************************************************************/
void podman_sigdiag_resp(int llid, char *line)
{
  t_cnt *cur;
  ...
  if (sscanf(line,
  "cloonsuid_podman_create_eth_done_ok brandtype=%s name=%s", type, name) == 2) 
    {
    cur = find_cnt(name);
    if (cur == NULL)
      KERR("ERROR %s", line);
  ...
  "cloonsuid_podman_resp_launch_ok brandtype=%s name=%s "
  "vm_id=%d image=%s pid=%d",
    ...
    if (cur == NULL)
      ...
      snprintf(req, MAX_PATH_LEN-1,
      "cloonsuid_podman_create_eth_done brandtype=%s name=%s",
  ...
  "cloonsuid_podman_delete_resp_ko brandtype=%s name=%s", type, name) == 2)
    {
    KERR("ERROR %s", line);
    cnt_free_cnt(name);
    }
  else if (sscanf(line,
  "cloonsuid_podman_delete_resp_ok brandtype=%s name=%s", type, name) == 2)
    {
    cnt_free_cnt(name);
    }
  else if ((sscanf(line,
  ...
  type, name, &vm_id, image) == 4) ||
           (sscanf(line,
  "cloonsuid_podman_killed brandtype=%s name=%s id=%s pid=%d", type, name, id, &pid) == 4) ||
           (sscanf(line,
  "cloonsuid_podman_delete_resp_ko brandtype=%s name=%s", type, name) == 2) ||
           (sscanf(line,
  "cloonsuid_podman_create_eth_done_ko brandtype=%s name=%s", type, name) == 2))
    {
    KERR("ERROR %s", line);
    cur = find_cnt(name);
    if (cur != NULL)
      ...
      memset(req, 0, MAX_PATH_LEN);
      snprintf(req, MAX_PATH_LEN-1,
      "cloonsuid_podman_ERROR brandtype=%s name=%s", type, name);
      if (send_sig_suid_power(llid, req))
...
/*****************************************************************************/
void podman_poldiag_resp(int llid, char *line)
{
  char name[MAX_NAME_LEN];
  char type[MAX_NAME_LEN];

  if (!mycmp(line, "cloonsuid_podman_resp_image_begin"))
    {
  ...
  else if (!mycmp(line, "cloonsuid_podman_resp_image_end"))
    {
...
/*****************************************************************************/
int podman_create(int llid, int vm_id, t_topo_cnt *cnt, char *agent)
{
  int result = -1;
  ...
  if (!podman_images_exists(cnt->brandtype, image))
    {
    ...
    cnt->brandtype, name, vm_id, image, cnt_dir, agent, startup_env);
    if (send_sig_suid_power(llid, req))
      {
...
/*****************************************************************************/
void podman_timer_beat(int llid)
{
  char req[MAX_PATH_LEN];
  snprintf(req, MAX_PATH_LEN-1, "cloonsuid_podman_req_imgs");
