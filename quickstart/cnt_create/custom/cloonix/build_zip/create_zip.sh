...
for i in "bin" "lib64" "root" "etc" "run" "tmp" \
  ...
  mkdir -p /root/cnt_cloonix/${i}
done

for i in ${LIST}; do
  cp ${i} /root/cnt_cloonix/bin
done

cp /lib64/ld-linux-x86-64.so.2 /root/cnt_cloonix/lib64

${HERE}/cplibdep /root/cnt_cloonix/bin /root/cnt_cloonix

cp /usr/lib/x86_64-linux-gnu/rsyslog/lmnet.so    /root/cnt_cloonix/usr/lib/x86_64-linux-gnu/rsyslog
cp /usr/lib/x86_64-linux-gnu/rsyslog/imklog.so   /root/cnt_cloonix/usr/lib/x86_64-linux-gnu/rsyslog
cp /usr/lib/x86_64-linux-gnu/rsyslog/imuxsock.so /root/cnt_cloonix/usr/lib/x86_64-linux-gnu/rsyslog


cat > /root/cnt_cloonix/etc/passwd << "EOF"
root:x:0:0:root:/root:/bin/bash
EOF
chmod 644 /root/cnt_cloonix/etc/passwd

cat > /root/cnt_cloonix/etc/group << "EOF"
root:x:0:
EOF
chmod 644 /root/cnt_cloonix/etc/group

cat > /root/cnt_cloonix/etc/rsyslog.conf << "EOF"
...
rm -vf ${BUNDLE} 
BUNDLE=${BUNDLE%%.*}
mv /root/${BUNDLE} /root/cnt_cloonix
chroot /root/cnt_cloonix /bin/sh -c "cd ${BUNDLE}; ./install_cloonix"
rm -rf /root/cnt_cloonix/${BUNDLE} 

cat > /root/cnt_cloonix/usr/bin/cloonix_startup_script.sh << "EOF"
#!/bin/bash
/bin/rsyslogd
...
EOF
chmod +x /root/cnt_cloonix/usr/bin/cloonix_startup_script.sh

cd /root/cnt_cloonix
zip -yr ../cnt_cloonix.zip .
