...
for i in ${LIST1}; do
  cloonix_cli ${NET} add cru tod${i} eth=sv   frr.zip --startup_env="NODE_ID=tod${i}"
  cloonix_cli ${NET} add cru nod${i} eth=sv   frr.zip --startup_env="NODE_ID=nod${i}"
  cloonix_cli ${NET} add cru cod${i} eth=vvvv frr.zip --startup_env="NODE_ID=cod${i}"
  for j in ${LIST2}; do
    cloonix_cli ${NET} add cru lod${i}${j} eth=sv frr.zip --startup_env="NODE_ID=lod${i}${j}"
    cloonix_cli ${NET} add cru nod${i}${j} eth=vv frr.zip --startup_env="NODE_ID=nod${i}${j}"
  done
done

for i in ${LIST3}; do
  cloonix_cli ${NET} add cru sod${i} eth=s  frr.zip --startup_env="NODE_ID=sod${i}"
done
